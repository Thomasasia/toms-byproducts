...
-- plate slag
add_byproduct(data.raw["recipe"]["iron-plate"], {{type="item", name="tbp-slag", amount=1}})
add_byproduct(data.raw["recipe"]["copper-plate"], {{type="item", name="tbp-slag", amount=1}})

-- steel slag modifications
add_byproduct_expensive_specify(data.raw["recipe"]["steel-plate"], {{type="item", name="tbp-slag", amount = 1, probability = 0.25}},"steel-plate")
...
add_byproduct(data.raw["recipe"]["uranium-processing"], {{type="item", name="tbp-slag", amount = 3}})

-- plastic bar waste
add_byproduct_specify(data.raw["recipe"]["plastic-bar"], {{type="item", name = "tbp-plastic-waste", amount = 1}}, "plastic-bar")

-- electronic circuit waste
add_byproduct_expensive_specify(data.raw["recipe"]["electronic-circuit"],{{type="item", name="tbp-spent-etchant", amount=1}}, "electronic-circuit")

---advanced circuit waste
add_byproduct_expensive_specify(data.raw["recipe"]["advanced-circuit"], {{type="item", name="tbp-spent-etchant", amount=1}, {type="item", name="tbp-plastic-waste", amount = 1}}, "advanced-circuit")

--processing unit waste
add_byproduct_expensive_specify(data.raw["recipe"]["processing-unit"], {{type="item", name="tbp-spent-etchant", amount=1}, {type="item", name="tbp-copper-sulfate", amount = 1}}, "processing-unit")

-- rocket control unit waste
add_byproduct(data.raw["recipe"]["rocket-control-unit"], {{type="item", name="tbp-spent-etchant", amount=1}, {type="item", name="tbp-copper-sulfate", amount = 1} , {type="item", name="tbp-plastic-waste", amount = 1}})

-- battery waste
add_byproduct_expensive(data.raw["recipe"]["battery"], {{type="item", name="tbp-copper-sulfate", amount = 1}})

-- military science waste
add_byproduct(data.raw["recipe"]["military-science-pack"], {{type="item", name = "tbp-slag", amount = 1}})


-- production science waste
add_byproduct(data.raw["recipe"]["production-science-pack"], {{type="item", name = "tbp-slag", amount = 1}})
